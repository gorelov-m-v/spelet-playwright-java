...
`RetryableExtension`. Оно читает свойство `test.retry` из файла `application-spelet.yml`
(например, `test.retry: 2`) и повторяет каждый тест указанное число раз. В этом случае
аннотация `@Retryable` становится необязательной.
